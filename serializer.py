...
def yaml_to_ini(schedule):
    output = []
    for day_name, info in schedule.items():
        output.append(f'[{day_name}]')
        output.append(f'date = {info["date"]}')
        lessons = info.get('lessons', [])
        output.append(f'lessons_count = {len(lessons)}')
        output.append('')
        for n, les in enumerate(lessons, 1):
            output.append(f'[{day_name}.lesson{n}]')
            for field in ['type', 'time', 'subject', 'teacher', 'room', 'building']:
                if field in les:
                    output.append(f'{field} = {les[field]}')
            output.append('')
...
if __name__ == '__main__':
    ...
    ini_text = yaml_to_ini(data)
    write_file('schedule.ini', ini_text)